...
void FieldLineDetectionLegacy::detect(const VisionInfoIn &info_in, const VisionInfoMiddle &info_middle, VisionInfoOut &info_out)
{
    ...
    RegionI *region_top = new RegionI(info_middle.full_regions[0]->zoomOut(2, true));
    RegionI *region_bot = new RegionI(info_middle.full_regions[0]->zoomOut(2, true));
    ...
    unsigned int *seed)
{
    ...
    for (l = landmarks.begin(); l != landmarks.end(); ++l, ++i)
    {
            ...
            (landmark.y() > 0) && (landmark.y() < TOP_IMAGE_ROWS))
        {
            ...
            boost::shared_ptr<Fovea> fovea(new Fovea(BBox(tl, br), density, true, true));
            fovea->generate(*combined_frame_);
