...
void construct_nets(std::vector<tiny_dnn::network<tiny_dnn::sequential>>& nets,
  ...
  nn1 << conv<3, 3>(300, 300, 3, 3, 64, tiny_dnn::padding::same)             // vgg.0
      << relu() << conv<3, 3>(300, 300, 3, 64, 64, tiny_dnn::padding::same)  // vgg.2
      << relu() << pool(300, 300, 64, 2, 2, false)
      << conv<3, 3>(150, 150, 3, 64, 128, tiny_dnn::padding::same)  // vgg.5
      << relu()
      << conv<3, 3>(150, 150, 3, 128, 128, tiny_dnn::padding::same)  // vgg.7
      << relu() << pool(150, 150, 128, 2, 2, false)
      << conv<3, 3>(75, 75, 3, 128, 256, tiny_dnn::padding::same)            // vgg.10
      << relu() << conv<3, 3>(75, 75, 3, 256, 256, tiny_dnn::padding::same)  // vgg.12
      << relu() << conv<3, 3>(75, 75, 3, 256, 256, tiny_dnn::padding::same)  // vgg.14
      << relu() << pool(75, 75, 256, 2, 2, true)
      << conv<3, 3>(38, 38, 3, 256, 512, tiny_dnn::padding::same)            // vgg.17
      << relu() << conv<3, 3>(38, 38, 3, 512, 512, tiny_dnn::padding::same)  // vgg.19
      << relu() << conv<3, 3>(38, 38, 3, 512, 512, tiny_dnn::padding::same)  // vgg.21
      << relu() << l2norm(38 * 38, 512, 1e-12, 20);
  nets.push_back(nn1);

  tiny_dnn::network<tiny_dnn::sequential> nn2;
  nn2 << pool(38, 38, 512, 2, 2, false)
      << conv<3, 3>(19, 19, 3, 512, 512, tiny_dnn::padding::same)            // vgg.24
      << relu() << conv<3, 3>(19, 19, 3, 512, 512, tiny_dnn::padding::same)  // vgg.26
      << relu() << conv<3, 3>(19, 19, 3, 512, 512, tiny_dnn::padding::same)  // vgg.28
      << relu() << pad(19, 19, 512, 1, 1) << pool(21, 21, 512, 3, 1, true)
      << pad(19, 19, 512, 6, 6)
      << conv<3, 3, 6, 6>(31, 31, 3, 512, 1024, tiny_dnn::padding::valid, true, 1, 1, 6,
              6)  // vgg.31
      << relu()
      << conv<1, 1>(19, 19, 1, 1024, 1024, tiny_dnn::padding::same)  // vgg.33
      << relu();
  nets.push_back(nn2);

  tiny_dnn::network<tiny_dnn::sequential> nn3;
  nn3 << conv<1, 1>(19, 19, 1, 1024, 256, tiny_dnn::padding::same)  // extras.0
      << relu() << pad(19, 19, 256, 1, 1)
      << conv<3, 3>(21, 21, 3, 256, 512, tiny_dnn::padding::valid, true, 2,
              2)  // extras.1
      << relu();
  nets.push_back(nn3);

  tiny_dnn::network<tiny_dnn::sequential> nn4;
  nn4 << conv<1, 1>(10, 10, 1, 512, 128, tiny_dnn::padding::same)  // extras.2
      << relu() << pad(10, 10, 128, 1, 1)
      << conv<3, 3>(12, 12, 3, 128, 256, tiny_dnn::padding::valid, true, 2,
              2)  // extras.3
      << relu();
  nets.push_back(nn4);

  tiny_dnn::network<tiny_dnn::sequential> nn5;
  nn5 << conv<1, 1>(5, 5, 1, 256, 128, tiny_dnn::padding::same)             // extras.4
      << relu() << conv<3, 3>(5, 5, 3, 128, 256, tiny_dnn::padding::valid)  // extras.5
      << relu();
  nets.push_back(nn5);

  tiny_dnn::network<tiny_dnn::sequential> nn6;
  nn6 << conv<1, 1>(3, 3, 1, 256, 128, tiny_dnn::padding::same)             // extras.6
      << relu() << conv<3, 3>(3, 3, 3, 128, 256, tiny_dnn::padding::valid)  // extras.7
      << relu();
  nets.push_back(nn6);

  // Locations
  tiny_dnn::network<tiny_dnn::sequential> nn7;
  nn7 << conv<3, 3>(38, 38, 3, 512, 16, tiny_dnn::padding::same);  // loc.0
  nets.push_back(nn7);

  tiny_dnn::network<tiny_dnn::sequential> nn8;
  nn8 << conv<3, 3>(19, 19, 3, 1024, 24, tiny_dnn::padding::same);  // loc.1
  nets.push_back(nn8);

  tiny_dnn::network<tiny_dnn::sequential> nn9;
  nn9 << conv<3, 3>(10, 10, 3, 512, 24, tiny_dnn::padding::same);  // loc.2
  nets.push_back(nn9);

  tiny_dnn::network<tiny_dnn::sequential> nn10;
  nn10 << conv<3, 3>(5, 5, 3, 256, 24, tiny_dnn::padding::same);  // loc.3
  nets.push_back(nn10);

  tiny_dnn::network<tiny_dnn::sequential> nn11;
  nn11 << conv<3, 3>(3, 3, 3, 256, 16, tiny_dnn::padding::same);  // loc.4
  nets.push_back(nn11);

  tiny_dnn::network<tiny_dnn::sequential> nn12;
  nn12 << conv<3, 3>(1, 1, 3, 256, 16, tiny_dnn::padding::same);  // loc.5
  nets.push_back(nn12);

  // Confidences
  tiny_dnn::network<tiny_dnn::sequential> nn13;
  nn13 << conv<3, 3>(38, 38, 3, 512, 4 * N_CLASSES,
               tiny_dnn::padding::same);  // conf.0
  nets.push_back(nn13);

  tiny_dnn::network<tiny_dnn::sequential> nn14;
  nn14 << conv<3, 3>(19, 19, 3, 1024, 6 * N_CLASSES,
               tiny_dnn::padding::same);  // conf.1
  nets.push_back(nn14);

  tiny_dnn::network<tiny_dnn::sequential> nn15;
  nn15 << conv<3, 3>(10, 10, 3, 512, 6 * N_CLASSES,
               tiny_dnn::padding::same);  // conf.2
  nets.push_back(nn15);

  tiny_dnn::network<tiny_dnn::sequential> nn16;
  nn16 << conv<3, 3>(5, 5, 3, 256, 6 * N_CLASSES, tiny_dnn::padding::same);  // conf.3
  nets.push_back(nn16);

  tiny_dnn::network<tiny_dnn::sequential> nn17;
  nn17 << conv<3, 3>(3, 3, 3, 256, 4 * N_CLASSES, tiny_dnn::padding::same);  // conf.4
  nets.push_back(nn17);

  tiny_dnn::network<tiny_dnn::sequential> nn18;
  nn18 << conv<3, 3>(1, 1, 3, 256, 4 * N_CLASSES, tiny_dnn::padding::same);  // conf.5
  nets.push_back(nn18);
