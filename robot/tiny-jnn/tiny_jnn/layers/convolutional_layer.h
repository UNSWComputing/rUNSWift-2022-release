// adapted from https://github.com/tiny-dnn/tiny-dnn/blob/master/tiny_dnn/layers/convolutional_layer.h
/*
...
#include "tiny_jnn/core/kernels/conv2d_op.h"
 ...
 public:
  ...
  void set_sample_count(size_t sample_count) override {
    ...
    cws_.prev_delta_padded_.resize(sample_count,
                                   vec_t(params_.in_padded.size(), tiny_dnn::float_t(0)));
  }
      ...
      for (size_t c = 0; c < params_.out.depth_; ++c) {
        ...
        for (size_t y = 0; y < params_.weight.height_; ++y) {
          for (size_t x = 0; x < params_.weight.width_; ++x) {
            ...
            const tiny_dnn::float_t w = W[idx];
 ...
 private:
  ...
  void conv_set_params(
    ...
    if (params_.pad_type == padding::same) {
      cws_.prev_delta_padded_.resize(
        1, vec_t(params_.in_padded.size(), tiny_dnn::float_t(0)));
    }
  ...
  void init_backend(const core::backend_t backend_type) {
    ...
    if (backend_type == core::backend_t::internal ||
      ...
      kernel_fwd_.reset(new Conv2dOp<window_width_tiny_jnn,
                                     window_height_tiny_jnn,
                                     w_dilation_tiny_jnn,
                                     h_dilation_tiny_jnn>(ctx));
...
using conv = convolutional_layer<window_width_tiny_jnn,
                                 window_height_tiny_jnn,
                                 w_dilation_tiny_jnn,
                                 h_dilation_tiny_jnn>;
