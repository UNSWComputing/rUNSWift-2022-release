...
RRCoord LocalisationTab::absToRR(AbsCoord obsAbs) {
   RRCoord obsRR;
   obsRR.setDistance(sqrt(SQUARE(obsAbs.x()-trueRobotPos.x()) + SQUARE(obsAbs.y()-trueRobotPos.y())));
   obsRR.heading() = NORMALISE(atan2(obsAbs.y()-trueRobotPos.y(),
                                     obsAbs.x()-trueRobotPos.x()) - trueRobotPos.theta());
   obsRR.setOrientation(NORMALISE(obsRR.heading() + trueRobotPos.theta() - obsAbs.theta() + M_PI));
   //obsRR.setOrientation(NORMALISE(obsAbs.theta() - trueRobotPos.theta()));
