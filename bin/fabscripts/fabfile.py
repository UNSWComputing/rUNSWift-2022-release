...
def restart_wireless():
   sudo("/etc/init.d/runswiftwireless restart || /etc/init.d/runswiftwireless.sh restart")

def which_wifi():
   run("cat /etc/wpa_supplicant/wpa_supplicant.conf /etc/wpa_supplicant.conf | grep ssid")
