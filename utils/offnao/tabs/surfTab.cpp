...
void SurfTab::keyPressEvent (QKeyEvent *event){
   ...
   switch(event->key()){
      ...
      case Qt::Key_Up:
         RobotVisionInfo[0].imageCoords.a.x() -= increment;
         RobotVisionInfo[0].imageCoords.b.x() += increment;
      break;

      case Qt::Key_Down:
         if (RobotVisionInfo[0].imageCoords.a.x() < RobotVisionInfo[0].imageCoords.b.x() + 2* increment){
            RobotVisionInfo[0].imageCoords.a.x() += increment;
            RobotVisionInfo[0].imageCoords.b.x() -= increment;
         }
      break;
...
void SurfTab::init() {
   ...
   this->RobotVisionInfo.push_back(RobotVisionInfo(robotlocation, RobotVisionInfo::rUnknown, robotBox));
  ...
  if(blackboard){
    ...
    switch (mode) {
        ...
        case mROBOT:{
          ...
          float robotRadius = static_cast<float>((RobotVisionInfo[0].imageCoords.b.x() - RobotVisionInfo[0].imageCoords.a.x())/SURF_SUBSAMPLE)/2.f;
          float robotCentre = static_cast<float>((RobotVisionInfo[0].imageCoords.b.x() + RobotVisionInfo[0].imageCoords.a.x())/SURF_SUBSAMPLE)/2.f;
...
void SurfTab::redraw() {
  ...
  if(blackboard){
      ...
      for(int i=0; i<(int)landmarks.size(); i++){
         if(landmarks[i].x*SURF_SUBSAMPLE >= RobotVisionInfo[0].imageCoords.a.x() &&
            landmarks[i].x*SURF_SUBSAMPLE <= RobotVisionInfo[0].imageCoords.b.x() )
   ...
   if (topFrame || saliency) {
      ...
      if(blackboard){
        ...
        if (mode == mROBOT){
            ...
            RobotVisionInfo[0].imageCoords.a.y() = horizon.first - SURF_HORIZON_WIDTH/2;
            RobotVisionInfo[0].imageCoords.b.y() = horizon.second + SURF_HORIZON_WIDTH/2;
