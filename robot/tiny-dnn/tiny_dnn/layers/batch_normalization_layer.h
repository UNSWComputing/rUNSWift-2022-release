...
class batch_normalization_layer : public layer {
 public:
  ...
  void forward_propagation(const std::vector<tensor_t *> &in_data,
    ...
    vec_t &weight = weight_; // added
    vec_t &bias = bias_;     // added
    ...
    for_i(in_data[0]->size(), [&](size_t i) {
      ...
      for (size_t j = 0; j < in_channels_; j++) {
        float_t m = mean[j];
        float_t w = weight[j]; // added
        float_t b = bias[j]; // added

        for (size_t k = 0; k < in_spatial_size_; k++) {
          *outptr++ = w * ((*inptr++ - m) / stddev_[j]) + b; // modified
        }
      }
    });
  ...
  void save(
    ...
    for (auto w : weight_) os << w << " "; // added
    for (auto b : bias_) os << b << " "; // added
  ...
  void load(std::istream &is,
    ...
    for (auto &w : weight_) is >> w; // added
    for (auto &b : bias_) is >> b; //added
  ...
  void load(const std::vector<float_t> &src, int &idx) override {
    Base::load(src, idx);
    for (auto &w : weight_) w   = src[idx++]; // added
    for (auto &b : bias_) b     = src[idx++]; // added
 ...
 private:
  ...
  void init() {
    ...
    weight_.resize(in_channels_); // added
    bias_.resize(in_channels_); // added
  }
